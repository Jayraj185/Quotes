...
class _AllQuotesPageState extends State<AllQuotesPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      backgroundColor: Colors.white,
      body: Obx(() => homeController.QuotesList.isNotEmpty
          ? ListView.builder(
        ...
        itemBuilder: (context, index) {
          return FocusedMenuHolder(
            menuItems: [
              FocusedMenuItem(title: Text("Update"), onPressed: (){
                homeController.Tabindex.value = 1;
                homeController.Quotecheck.value = 1;
                homeController.DropdownValue = homeController.QuotesCategory;
                homeController.txtUpdateQuotes.value = TextEditingController(text: homeController.QuotesList[index]);
                homeController.QuoteId.value = homeController.QuotesIdList[index];
                print("====== Drop ${homeController.DropdownValue.value} Id ${homeController.QuoteId.value} List = ${homeController.QuotesIdList[index]} Cateid = ${homeController.CategoryId}");
                Get.toNamed('Tab');
              }),
              FocusedMenuItem(title: Text("Delete"), onPressed: () async {
                QuotesDatabase.quotesDatabase.DeleteQuoteData(id: homeController.QuotesIdList[index]);
